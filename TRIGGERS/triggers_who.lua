...
local function setup(args)
	...
	tempTriggers.playersOnline3 =
	tempRegexTrigger("^(?:> )*  If a tree falls in the forest, and none of the roughly \\d+ [\\s\\S]*"
		,[[
			args = {}
			Events.raiseEvent("whoEvent", args)
		]])
	...
	tempRegexTrigger("^(?:> )* *Most people seem to be in (?:\\w+) right now\\."
		...
		tempTriggers.whoWhere2 =
		tempRegexTrigger("^(?:> )* *Most people seem to be in the (?:\\w+) right now\\."
			,[[
				args = {}
				Events.raiseEvent("whoEvent", args)
			]])
